...
Package [app](./pkg/app/app.go) holds the core generic abstractions. An `App` is defined as a trie + notFound handler.
...
Package [http](./pkg/http/http.go) implements a thin adapter for [net/http](https://pkg.go.dev/net/http) to demonstrate how the "request model" maps to other domains, such as HTTP.
